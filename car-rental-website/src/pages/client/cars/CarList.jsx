...
const CarList = () => {
    ...
    return cars
      .filter((car) => {
        ...
        if (filters.search) {
          ...
          if (
            !car.tenXe?.toLowerCase().includes(searchTerm) &&
        ...
        if (
          filters.hangXe &&
          (!car.HangXe || car.HangXe.tenHangXe !== filters.hangXe)
        ) {
          return false;
        }
        ...
        if (
          filters.loaiXe &&
          (!car.LoaiXe || car.LoaiXe.tenLoaiXe !== filters.loaiXe)
        ) {
          return false;
        }
        ...
        if (
          filters.namSanXuat &&
          car.namSanXuat !== parseInt(filters.namSanXuat)
        ) {
          return false;
        }
        ...
        if (
          filters.trangThai !== "" &&
          car.trangThai !== parseInt(filters.trangThai)
        ) {
          return false;
        }
  ...
  const getHangXe = () => {
    return cars && cars.length > 0
      ? [
          ...new Set(
            cars.filter((car) => car.HangXe).map((car) => car.HangXe.tenHangXe)
          ),
        ]
      : [];
  };
  ...
  const getLoaiXe = () => {
    return cars && cars.length > 0
      ? [
          ...new Set(
            cars.filter((car) => car.LoaiXe).map((car) => car.LoaiXe.tenLoaiXe)
          ),
        ]
      : [];
  };
      ...
      <div className="bg-gradient-to-r from-red-700 to-red-500 py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <div className="flex justify-end mb-4">
            <Link
              to="/cart"
              className="flex items-center bg-white/20 backdrop-blur px-4 py-2 rounded-full text-white"
            >
            ...
            {filteredCars.length > 0 ? (
              ...
              <div className="bg-yellow-50 border border-yellow-200 p-8 rounded-lg text-center">
                ...
                <p className="text-yellow-700 mb-4">
                  Không có xe nào phù hợp với bộ lọc của bạn. Vui lòng thử lại
